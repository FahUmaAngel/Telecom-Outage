...
export default function AdminPage() {
    ...
    const loadAdminData = async () => {
        try {
            ...
            addToast(
                lang === "sv" ? "Kunde inte hämta admin-data" : "Failed to load admin data",
                "error"
            );
        } finally {
            setLoading(false);
    ...
    const loadOutages = async (nextPage = 0, overrides = {}) => {
        ...
        try {
            ...
            addToast(
                lang === "sv" ? "Kunde inte hämta driftstörningar" : "Failed to fetch outages",
                "error"
            );
        } finally {
    ...
    return (
        <div style={{ padding: 24, maxWidth: 1280, margin: "0 auto" }}>
                ...
                <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                        <thead>
                            <tr>
                                <th align="left">ID</th>
                                <th align="left">{lang === "sv" ? "Operatör" : "Operator"}</th>
                                <th align="left">{lang === "sv" ? "Titel" : "Title"}</th>
                                <th align="left">{lang === "sv" ? "Status" : "Status"}</th>
                                <th align="left">{lang === "sv" ? "Åtgärd" : "Action"}</th>
                            </tr>
                        </thead>
                        <tbody>
                            {reports.map((report) => (
            ...
            <section style={{ marginBottom: 32 }}>
                <h2>{lang === "sv" ? "Driftstörningar" : "Outages"}</h2>
            ...
            {editingOutage && (
                <section style={{ border: "1px solid var(--border-color)", borderRadius: 8, padding: 16 }}>
                    <h2>{lang === "sv" ? "Redigera driftstörning" : "Edit outage"} #{editingOutage.id}</h2>
