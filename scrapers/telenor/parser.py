...
def parse_telenor_outage(data: Dict) -> Optional[Dict]:
    """
    Parse Telenor outage.
...
def parse_telenor_outages(raw_outages: List) -> List[Dict]:
    parsed = []
    for raw in raw_outages:
        try:
            data = raw.raw_data if hasattr(raw, 'raw_data') else raw
            outage = parse_telenor_outage(data)
            if outage:
